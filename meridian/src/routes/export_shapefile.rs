//! POST /v1/export/shapefile — Export GeoJSON features as an ESRI Shapefile (.shp).
//! Accepts a multipart form upload with a GeoJSON FeatureCollection.
//! Output is a ZIP archive containing all Shapefile components (.shp, .shx, .dbf, .prj).
...
#[derive(ToSchema)]
pub struct ExportShapefileParams {
...
pub struct ExportShapefileResponse {
    pub request_id: String,
    pub price_usd: f64,
    /// ZIP archive bytes (containing .shp, .shx, .dbf, .prj) as base64
...
#[utoipa::path(
    post,
    path = "/v1/export/shapefile",
    tag = "GIS",
    request_body(
        content_type = "multipart/form-data",
        description = "Multipart form: `geojson` (GeoJSON string), optional `input_crs` (default EPSG:4326)",
        content = ExportShapefileParams
    ),
    responses(
        (status = 200, description = "Shapefile ZIP output", body = ExportShapefileResponse),
...
pub async fn export_shapefile(
...
) -> Result<Json<ExportShapefileResponse>, AppError> {
    ...
    let t0 = Instant::now();
    metrics::record_request("export-shapefile", "received");

    payment_gate("export-shapefile", file_size, price, &request_id, &headers, &state).await?;
    ...
    let result = timeout(OP_TIMEOUT, tokio::task::spawn_blocking(move || {
        run_export_shapefile(&uuid, &geojson_bytes, &input_crs)
    }))
    .await
    ...
    metrics::record_request("export-shapefile", "ok");
    metrics::record_request_duration("export-shapefile", t0.elapsed().as_secs_f64());
    ...
    let response_body = ExportShapefileResponse {
        request_id,
        price_usd: price,
        zip_base64: encoded,
        filename: "output.zip".to_string(),
    };

    Ok(Json(response_body))
}

fn run_export_shapefile(uuid: &str, geojson_bytes: &[u8], _input_crs: &str) -> Result<Vec<u8>, AppError> {
    ...
    let driver = DriverManager::get_driver_by_name("ESRI Shapefile")
        .map_err(|e| AppError::BadRequest(format!("ESRI Shapefile driver not available: {e}")))?;

    // Shapefile driver creates multiple files: .shp, .shx, .dbf, .prj, .cpg, .sbn, .sbx
    // We pass the base path (no extension); GDAL appends the extensions
    let shapefile_basename = format!("{}_output", uuid);
    let shapefile_dir = tmp_dir.path().join(&shapefile_basename);
    std::fs::create_dir_all(&shapefile_dir)
        .map_err(|e| AppError::Internal(anyhow::anyhow!("Create shapefile dir: {e}")))?;

    let out_path = shapefile_dir.join(&shapefile_basename);
    let mut dst_ds = driver
        .create_vector_only(out_path.to_str().unwrap_or("output"))
        .map_err(|e| AppError::Internal(anyhow::anyhow!("Create Shapefile dataset: {e}")))?;

    // Use the original layer name from the source (or a safe default)
    ...
    let dst_layer = dst_ds.create_layer(layer_opts)
        .map_err(|e| AppError::Internal(anyhow::anyhow!("Create Shapefile layer: {e}")))?;
    ...
    // Collect all shapefile component files and zip them
    let component_extensions = ["shp", "shx", "dbf", "prj", "cpg", "sbn", "sbx"];
    let mut zip_buffer = Vec::new();
    {
        ...
        for ext in component_extensions {
            let file_path = shapefile_dir.join(format!("{}.{}", shapefile_basename, ext));
            if file_path.exists() {
                let file_name = format!("{}.{}", shapefile_basename, ext);
                let file_bytes = std::fs::read(&file_path)
                    .map_err(|e| AppError::Internal(anyhow::anyhow!("Read shapefile component {}: {e}", file_name)))?;
                zip.start_file(&file_name, options)
                    .map_err(|e| AppError::Internal(anyhow::anyhow!("Zip start_file {}: {e}", file_name)))?;
                zip.write_all(&file_bytes)
                    .map_err(|e| AppError::Internal(anyhow::anyhow!("Zip write {}: {e}", file_name)))?;
            }
        }
...
/// Registers the /v1/export/shapefile route.
pub fn routes() -> Router {
    Router::new().route("/v1/export/shapefile", post(export_shapefile))
}
